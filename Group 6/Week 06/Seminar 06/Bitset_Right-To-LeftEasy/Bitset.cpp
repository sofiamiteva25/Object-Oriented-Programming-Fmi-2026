...
bool Bitset::contains(size_t number) const
{
    ...
    return ((data[bucket] & (1 << position)) != 0);
}
...
void Bitset::save(std::ofstream& ofs) const {
    if (!ofs.is_open()) {
        return;
    }

    ofs.write((const char*)&bucketsCount, sizeof(size_t));
    ofs.write(data, bucketsCount);
}
Bitset read(std::ifstream& ifs) {
    if (!ifs.is_open()) {
        return Bitset(0);
    }
    ...
    Bitset bitset(size * 8);

    bitset.limit = size * 8;
    bitset.bucketsCount = size;

    delete[] bitset.data;
    bitset.data = new char[size];

    ifs.read(bitset.data, size);
}
